...
class TestSystemModes:
    ...
    def test_system_modes_with_all_enabled(self):
        """Test SystemModes with all modes enabled."""
        data = {
            "off": "yes",
            "heat": "yes",
            "cool": "yes",
        ...
        assert modes.aux is True
        assert modes.auto is True

    def test_system_modes_with_partial_enabled(self):
    ...
    def test_system_modes_with_all_disabled(self):
        """Test SystemModes with all modes disabled."""
        data = {
            "off": "no",
            "heat": "no",
            "cool": "no",
...
class TestCirculatingFanCapabilities:
    ...
    def test_circulating_fan_capabilities_with_data(self):
    ...
    def test_circulating_fan_capabilities_not_capable(self):
        """Test CirculatingFanCapabilities when not capable."""
        data = {
            "capable": "no",
            "max_duty_cycle": 0,
            "min_duty_cycle": 0,
    ...
    def test_circulating_fan_capabilities_with_boolean_values(self):
        """Test CirculatingFanCapabilities with boolean values instead of strings."""
        data = {
            "capable": True,
            "max_duty_cycle": 100,
            "min_duty_cycle": 10,
...
class TestFanModes:
    ...
    def test_fan_modes_with_all_enabled(self):
        """Test FanModes with all modes enabled."""
        data = {"auto": "yes", "on": "yes", "smart": "yes"}
        modes = FanModes(data)
        assert modes.auto is True
        assert modes.on is True
        assert modes.smart is True

    def test_fan_modes_with_partial_enabled(self):
    ...
    def test_fan_modes_with_all_disabled(self):
        """Test FanModes with all modes disabled."""
        data = {"auto": "no", "on": "no", "smart": "no"}
        modes = FanModes(data)
        assert modes.auto is False
        assert modes.on is False
        assert modes.smart is False
...
class TestHumidityCapabilities:
    ...
    def test_humidity_capabilities_with_humidification_data(self):
    ...
    def test_humidity_capabilities_with_dehumidification_data(self):
        """Test HumidityCapabilities for dehumidification."""
        data = {"step": 5, "min": 40, "max": 95, "types": ["overcooling"]}
        humidity = HumidityCapabilities(data)
        assert humidity.step == 5
        assert humidity.min == 40
        assert humidity.max == 95
        assert humidity.types == ["overcooling"]
    ...
    def test_humidity_capabilities_with_multiple_types(self):
        """Test HumidityCapabilities with multiple types."""
        data = {
            "step": 5,
            "min": 40,
            "max": 95,
            "types": ["overcooling", "humidifier"],
        }
        humidity = HumidityCapabilities(data)
        assert len(humidity.types) == 2
        assert "overcooling" in humidity.types
        assert "humidifier" in humidity.types
...
class TestCapabilities:
    ...
    def test_capabilities_with_valid_data(self, mock_json):
        """Test Capabilities with valid data from sample."""
        cap_data = mock_json.get("capabilities", {})
        capabilities = Capabilities(cap_data)
        assert capabilities.continuous_backlight is not None
        assert capabilities.display_humidity is not None
        assert capabilities.display_time is not None
        assert capabilities.degrees_fc is not None
        assert capabilities.keypad_lockout is not None
    ...
    def test_capabilities_string_representation(self, mock_json):
        """Test Capabilities string representation."""
        cap_data = mock_json.get("capabilities", {})
        capabilities = Capabilities(cap_data)
        cap_str = str(capabilities)
        assert "Capabilities" in cap_str
        assert "heat_range" in cap_str
        assert "cool_range" in cap_str
