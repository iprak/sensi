...
class State:
    ...
    def __init__(self, data: dict) -> None:
        ...
        self.battery_voltage = to_float(data.get("battery_voltage"), 0.0)
        ...
        self.cool_min_temp = to_int(data.get("cool_min_temp"), COOL_MIN_TEMPERATURE)
        self.current_cool_temp = to_int(data.get("current_cool_temp"), 0)
        self.current_heat_temp = to_int(data.get("current_heat_temp"), 0)
        ...
        self.display_temp = to_float(data.get("display_temp"), 0.0)
        self.display_time = to_bool(data.get("display_time"))
        self.fan_mode = try_parse_enum(FanMode, data.get("fan_mode", FanMode.UNKNOWN))
        self.heat_max_temp = to_int(data.get("heat_max_temp"), HEAT_MAX_TEMPERATURE)
        self.humidity = to_int(data.get("humidity"), 0)
